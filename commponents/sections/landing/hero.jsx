...
export default function hero() {
  const { language } = useMainContext(); // replace with your actual language source

  return (
    <div className="flex flex-col  overflow-hidden gap-5  relative   w-dvw h-svh items-center   justify-center ">
      <HeroBg />
      <span
    ...
    text-transparent  text-[clamp(0.8rem,10vw,5rem)] 3xl:text-[clamp(1rem,10vw,15rem)]! md:text-[clamp(1rem,10vw,5rem)] lg:text-[clamp(1rem,15vw,7rem)]"
      ...
      >
        ...
        <span className="relative z-10 pointer-events-none text-[#403E37] font-bold">
          {t("landing.hero.preRegisterBtn", language)}
        </span>
      </Link>
